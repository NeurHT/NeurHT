...
from pretrainedmodels.models.torchvision_models import modify_resnets
...
# --------------- Bagnet-33 ---------------
def bagnet33_(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    ...
    if pretrained:
        checkpoint = model_zoo.load_url(model_urls['bagnet33'])
        checkpoint['last_linear.weight'] = checkpoint['fc.weight']
        checkpoint['last_linear.bias'] = checkpoint['fc.bias']
...
def bagnet33(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    ...
    if pretrained == 'imagenet':
        model = bagnet33_(True, strides, **kwargs)
    elif pretrained in (None, False):
        model = bagnet33_(False, strides, **kwargs)
    else:
        raise NotImplementedError('Pretrained={} not supported'.format(pretrained))
...
# --------------- Bagnet-17 ---------------
def bagnet17_(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    ...
    if pretrained:
        checkpoint = model_zoo.load_url(model_urls['bagnet17'])
        checkpoint['last_linear.weight'] = checkpoint['fc.weight']
        checkpoint['last_linear.bias'] = checkpoint['fc.bias']
        del checkpoint['fc.weight']
        del checkpoint['fc.bias']
...
def bagnet17(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    ...
    """
    if pretrained == 'imagenet':
        model = bagnet17_(True, strides, **kwargs)
    elif pretrained in (None, False):
        model = bagnet17_(False, strides, **kwargs)
    else:
        raise NotImplementedError('Pretrained={} not supported'.format(pretrained))
    # model = modify_resnets(model)
    # model.last_linear = model.fc
...
# --------------- Bagnet-9 ---------------
def bagnet9_(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    ...
    if pretrained:
        checkpoint = model_zoo.load_url(model_urls['bagnet9'])
        checkpoint['last_linear.weight'] = checkpoint['fc.weight']
        checkpoint['last_linear.bias'] = checkpoint['fc.bias']
        del checkpoint['fc.weight']
        del checkpoint['fc.bias']
...
def bagnet9(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    ...
    """
    if pretrained == 'imagenet':
        model = bagnet9_(True, strides, **kwargs)
    elif pretrained in (None, False):
        model = bagnet9_(False, strides, **kwargs)
    else:
        raise NotImplementedError('Pretrained={} not supported'.format(pretrained))
    # model = modify_resnets(model)
    # model.last_linear = model.fc
