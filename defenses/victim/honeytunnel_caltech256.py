...
class HoneyTunnel(Blackbox):
    def __init__(self, pos=10, size=5, target_class=9, hard_label=0, *args, **kwargs):
        ...
        self.model.last_linear.register_forward_hook(forward_hook)



        with torch.no_grad():
            self.mix_data = torch.load('mix_dataset_caltech256.pt').to(self.device)
            self.mix_features = []
    ...
    def __call__(self, x, stat = True, return_origin = False):
        ...
        #similarity = self.compute_similarity(x)
            ...
            similarity = 1.05 - similarity

        # detect extraction queries
        extraction_mask = y_v.max(dim=1).values < 0.1
        ...
        benign_mask = y_v.max(dim=1).values > 0.95
        similarity[benign_mask] = similarity[benign_mask]**4
